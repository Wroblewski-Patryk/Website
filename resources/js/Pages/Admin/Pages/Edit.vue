        ...
        <div class="h-[calc(100vh-64px)] flex flex-col overflow-hidden bg-base-300">
            ...
            <div class="flex items-center justify-between px-6 py-2 bg-base-100 border-b border-white/5 shadow-md z-20">
                <div class="flex items-center gap-4">
                    <button @click="$inertia.visit(route('admin.pages.index'))" class="btn btn-ghost btn-sm">
            ...
            <div class="flex-1 flex overflow-hidden">
                <!-- Left Sidebar: Palette & Page Settings -->
                    ...
                    <div class="flex-1 overflow-y-auto p-4 custom-scrollbar">
                        ...
                        <div v-show="leftTab === 'blocks'" class="space-y-6">
                            ...
                            <section>
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <button @click="store.addBlock('hero')" class="btn btn-outline btn-sm h-16 flex flex-col gap-1 border-white/10 hover:bg-white/5">
                                        <i class="fas fa-star text-lg"></i>
                                        <span class="text-[10px]">Hero</span>
                                    </button>
                                    ...
                                    <button @click="store.addBlock('contact_form')" class="btn btn-outline btn-sm h-16 flex flex-col gap-1 border-white/10 hover:bg-white/5">
                                        <i class="fas fa-envelope text-lg"></i>
                                        <span class="text-[10px]">Form</span>
                                    </button>
                                </div>
                            </section>
                        </div>
                        ...
                        <div v-show="leftTab === 'settings'" class="space-y-4">
                            ...
                            <div class="divider text-xs opacity-30 uppercase tracking-widest">Templates</div>
                            <div class="form-control">
                                <label class="label"><span class="label-text">Header Override</span></label>
                                <select v-model="form.header_override_id" class="select select-bordered select-sm">
                                    <option :value="null">Global Default</option>
                                    <option v-for="t in templates.header || []" :key="t.id" :value="t.id">{{ t.name }}</option>
                                </select>
                            </div>
                            <div class="form-control">
                                <label class="label"><span class="label-text">Footer Override</span></label>
                                <select v-model="form.footer_override_id" class="select select-bordered select-sm">
                                    <option :value="null">Global Default</option>
                    ...
                    <div :class="[
                        ...
                        <div class="p-0">
                            ...
                            <div v-if="store.blocks.length === 0" class="h-96 flex flex-col items-center justify-center border-2 border-dashed border-base-content/10 m-10 rounded-2xl">
                                ...
                                <p class="opacity-40">Start building your page by adding blocks</p>
                            </div>
                        </div>

                        <!-- Canvas Footer Placeholder -->
...
watch(() => form.title.pl, (newTitle) => {
    if (!props.page?.id || !form.slug.pl) {
        form.slug.pl = newTitle.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');
    }
});
