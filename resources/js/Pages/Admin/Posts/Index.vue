...
const columns = [
    ...
    { key: 'title', label: t('admin.posts.title_field', 'Title'), sortable: true },
    { key: 'slug', label: t('admin.posts.slug_field', 'Slug'), sortable: true, optional: true },
    { key: 'status', label: t('admin.posts.status_field', 'Status'), sortable: true },
...
<template>
    <Head :title="t('admin.posts.management_title', 'Manage Posts')" />
    <AdminLayout>
        <ResourceTable
            :title="t('admin.menu.posts', 'Posts')"
            :description="t('admin.posts.description', 'Manage your blog posts and announcements.')"
        ...
        >
            ...
            <template #cell-actions="{ item }">
                <div class="flex justify-end gap-2">
                    <a :href="`/${activeLocale}/blog/${t(item.slug)}`" target="_blank" class="btn btn-sm btn-ghost btn-square hover:bg-info/10 hover:text-info transition-all" :title="t('admin.common.preview', 'Preview')">
                        <PhEye weight="regular" class="w-4 h-4" />
                    </a>
                    <Link :href="route('admin.posts.edit', item.id)" class="btn btn-sm btn-ghost btn-square hover:bg-primary/10 hover:text-primary transition-all" :title="t('admin.common.edit', 'Edit')">
