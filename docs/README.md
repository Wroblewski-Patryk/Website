# Docs Index

## Start Here
- `overview.md`
- `product/product.md`
- `architecture/system-architecture.md`
- `planning/mvp-next-commits.md`

## Governance
- `governance/working-agreements.md`
- `governance/language-policy.md`
- `governance/template-usage.md`
- `governance/subagent-delegation-policy.md`
- `governance/repository-structure-policy.md`

## ADR
- `adr/README.md`
- `adr/0001-translation-scanning-system.md`
- `adr/0001-template-governance-baseline.md`

## Product
- `product/product.md`
- `product/roadmap.md`
- `product/mvp_scope.md`
- `product/localization.md`
- `product/known-limits.md`
- `product/glossary.md`

## Architecture
- `architecture/system-architecture.md`
- `architecture/modules.md`
- `architecture/tech-stack.md`
- `architecture/admin-frontend-architecture.md`
- `architecture/seo-route-contracts.md`

## Module Deep Dives
- `modules/README.md`

## Planning
- `planning/mvp-execution-plan.md`
- `planning/mvp-next-commits.md`
- `planning/v1-live-release-plan.md`
- `planning/open-decisions.md`

## Engineering
- `engineering/local-development.md`
- `engineering/testing.md`
- `engineering/user-journeys.md`
- `engineering/manual-smoke-checklist.md`
- `engineering/universal-cross-app-test-checklist.md`

## Operations
- `operations/mvp-ops-runbook.md`
- `operations/control-center-10s-checklist.md`
- `operations/release-candidate-checklist.md`
- `operations/post-release-monitoring.md`
- `operations/operator-handbook.md`

## Security
- `security/security-and-risk.md`

## UX
- `ux/ui-ux-foundation.md`
- `ux/ux-ui-mcp-collaboration.md`
- `ux/stitch-mcp-playbook.md`
- `ux/ux-parity-evidence-log.md`
